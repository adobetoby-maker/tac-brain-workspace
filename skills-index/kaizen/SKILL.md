...
## The 7-Step Kaizen Process

A structured approach to running a kaizen event or improvement cycle.
...
- Metrics that are trending wrong (error rate, build time, cycle time)
- "4cpl Kaizen Success" — Cost, Customer, Culture, Compliance pain signals

**Output:** One clearly stated problem with measurable baseline.

### Step 2: Map and Analyze the Current Process

Document the workflow as it actually works, not how it's supposed to work.

**Techniques:**
- Value stream mapping — draw every step, handoff, and wait time
- Identify the 7 wastes: overproduction, waiting, transport, over-processing, inventory, motion, defects
- Time each step — where does the process stall?

**Output:** Visual map of current state with bottlenecks highlighted.

### Step 3: Determine the Root Cause

Don't treat symptoms. Find the actual cause.
...
**Output:** Documented root cause that, if fixed, eliminates the problem.

### Step 4: Develop an Optimal Solution

Brainstorm with the people closest to the problem — not just management.

**Principles:**
- Generate 5+ ideas before evaluating any
- Prefer simple, reversible changes over complex ones
- Involve stakeholders who will be affected
- Check: does this address the root cause, or a symptom?

**Output:** Chosen solution with rationale and expected improvement.
...
**Sequence:**
1. Pilot on one team, one module, one environment
2. Observe for one cycle (sprint, week, deploy)
3. Measure against baseline from Step 1
4. If results confirm improvement → roll out fully

**Code context:** Branch the change, run CI, deploy to staging. Don't merge to main until the pilot confirms the fix.

### Step 6: Study the Results

Measure with hard data — not intuition.
...
**Honest evaluation:** If the fix didn't move the metric, treat this as new data — return to Step 3 with updated information. That is not failure; it is the method working.

**Output:** Written results with data, including what did and didn't improve.
...
**Actions:**
- Formalize the new process in documentation (CLAUDE.md, SKILL.md, team wiki)
- Update onboarding materials so new team members start with the improvement
- Train staff on the new standard — show, don't just tell
- Set up a monitoring signal (alert, metric, periodic review) that catches regression early
- Celebrate the win — makes the next kaizen cycle easier to start

**Output:** Updated standard, trained team, monitoring in place.

---

**Loop back to Step 1.** The 7-step process is a cycle, not a checklist. Every improvement reveals the next opportunity.
